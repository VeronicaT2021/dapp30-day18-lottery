...
function App() {
  ...
  async function updateBet() {
    const [state, betCount, betSize, admin] = await Promise.all([
      contract.methods.currentState().call(),
      contract.methods.betCount().call(),
      contract.methods.betSize().call(),
      contract.methods.admin().call()
    ])
    setBet({ state, betCount, betSize, admin });
  }
  ...
  async function doBet() {
    await contract.methods
      .bet()
      .send({ from: accounts[0], value: bet.betSize })
    await updatePlayers();
  };
  ...
  return (
    <div className="container">
      ...
      {bet.state === "1" && accounts[0].toLowerCase() === bet.admin.toLowerCase() ? (
        <div className="row">
          <div className="col-sm-12">
            <h2>Cancel bet</h2>
            <button
